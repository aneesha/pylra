...
class GoogleSearch(object):
    ...
    def getresultId(self, lastresult):
        return "-".join([lastresult.title, lastresult.url, lastresult.desc])
    ...
    def get_results(self):
        ...
        if (hasattr(self, 'last_result') and self.last_result!=None):
            i=0
            for result in results:
                if (self.last_result==self.getresultId(result)):
                    results=results[i+1:]
                    break;
                i+=1
        if (len(results)>0): self.last_result=self.getresultId(results[-1])
        return results
    ...
    def _get_results_page(self, start_result_number=0):
        if self._page == 0:
